...
The lower number of packets sent (blue) when network duty is between 0 and 0.4 is due to backpressure effects and the dropped packets corresponding to the solid red line are dropped but reinjected so in reality they are not being lost.
...
Adding the line \verb|e.burst_phase = None| (to \verb|bursting.py|) before \verb|e.run()| is called , the packet generators will still generate bursts but they will be at random phases to each other, i.e., not bursting all at the same time. This normally eliminates packet dropping for all but the most extreme burstiness. The result of this experiment is shown in \textbf{Figure~\ref{fig:bursting_random}}
